...
class MuiAutocompleteTest {
    ...
    /**
     * Mocks findComponents chain: locator.locator(any).all() → list of locators
     */
    private void mockFindComponents(Locator... locators) {
        Locator childLocator = mock(Locator.class);
        when(locator.locator(anyString())).thenReturn(childLocator);
        when(childLocator.all()).thenReturn(List.of(locators));
    }
